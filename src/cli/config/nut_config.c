...
static bool has_header(char **headers, size_t count, const char *key) {
    ...
    for (size_t i = 0; i < count; i++) {
        if (nut_strncasecmp(headers[i], key, key_len) == 0 && headers[i][key_len] == ':') {
            return true;
        }
    }
    return false;
...
void nut_config_load_and_merge(CommonArgs *args) {
    char *config_path = getenv("NUT_CONFIG");
    ...
    if (!f) {
        ...
        if (explicitly_set) {
            nut_diag_warn("could not open config file '%s': %s", config_path, strerror(errno));
        }
        free(allocated_path);
        return;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        char *trimmed = nut_utils_trim(line);
        if (strlen(trimmed) == 0 || trimmed[0] == '#' || trimmed[0] == ';') {
            continue;
        }

        if (trimmed[0] == '[') {
            if (nut_strcasecmp(trimmed, "[defaults]") == 0) {
                section = 1;
            } else if (nut_strcasecmp(trimmed, "[headers]") == 0) {
                section = 2;
            } else {
                section = 0;
            }
        ...
        if (eq) {
            *eq = '\0';
            char *key = nut_utils_trim(trimmed);
            char *val = nut_utils_trim(eq + 1);
            strip_quotes(val);

            if (section == 1) {
                // [defaults]
                if (nut_strcasecmp(key, "timeout") == 0 && !args->is_set.timeout) {
                    args->timeout = strtoul(val, NULL, 10);
                } else if (nut_strcasecmp(key, "connect_timeout") == 0 && !args->is_set.connect_timeout) {
                    args->connect_timeout = strtoul(val, NULL, 10);
                } else if (nut_strcasecmp(key, "follow_redirects") == 0 && !args->is_set.location) {
                    if (nut_strcasecmp(val, "true") == 0) {
                        args->location = true;
                    }
                } else if (nut_strcasecmp(key, "user_agent") == 0 && !args->is_set.user_agent) {
