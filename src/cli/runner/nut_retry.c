...
int execute_with_retry(NutCtx *ctx, NutRequest *req, const CommonArgs *common, nut_http_response_t **out_res, char **out_effective_url, NutOperationStats *stats) {
    ...
    int engine_err = NUT_OK;

    for (unsigned int attempt = 0; attempt <= max_retries; attempt++) {
        if (*out_res) { nut_http_response_free(*out_res); *out_res = NULL; }
        if (*out_effective_url) { free(*out_effective_url); *out_effective_url = NULL; }

        engine_err = nut_engine_execute_request(ctx, req, out_res, out_effective_url, stats);

        bool should_retry = false;
        if (engine_err == NUT_OK && *out_res) {
            should_retry = ((*out_res)->status_code >= 500);
            if (should_retry && attempt < max_retries && !common->silent) {
                nut_diag_warn("HTTP %d. Retrying...", (*out_res)->status_code);
            }
        } else {
            should_retry = (engine_err == NUT_ERR_TIMEOUT || engine_err == NUT_ERR_NETWORK);
            if (should_retry && attempt < max_retries && !common->silent) {
                nut_diag_warn("Request failed (error %d). Retrying...", engine_err);
            }
        }

        if (!should_retry) break;
        if (attempt < max_retries) nut_sleep_ms(delay_sec * 1000);
    }
    return engine_err;
}
