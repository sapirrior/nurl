...
static char *nut_normalize_url(const char *raw) {
    if (!raw) return NULL;
...
static void nut_cli_infer_method(CommonArgs *args) {
...
static int set_arg_str(char **dest, const char *val, const char *name) {
    if (val[0] == '\0') { nut_diag_err("--%s value cannot be empty.", name); return -1; }
    if (*dest) free(*dest);
    if (!(*dest = strdup(val))) { nut_diag_err("out of memory while processing --%s.", name); return -1; }
    return 0;
}
...
static int append_arg_str(char ***array, size_t *count, const char *val, const char *name) {
    ...
    if (!temp || !(temp[*count] = strdup(val))) { nut_diag_err("out of memory while processing --%s.", name); if (temp) *array = temp; return -1; }
...
static int parse_non_negative_int(const char *optarg, const char *name) {
    ...
    if (*end != '\0' || v > 2147483647) {
        nut_diag_err("--%s must be a valid non-negative integer.", name);
        return -1;
    }
    return (int)v;
}
    ...
    while ((opt = getopt_long(argc, argv, "u:d:jt:LH:o:ivshkb:c:w:Vx:A:e:fX:DI", long_options, NULL)) != -1) {
        switch (opt) {
            ...
            case 'H': {
                ...
                if (!colon) { nut_diag_err("invalid header '%s'", optarg); return -1; }
                size_t key_len = colon - optarg;
                if (key_len == 0) { nut_diag_err("header key cannot be empty."); return -1; }
                for (size_t i = 0; i < key_len; i++) {
                    if (isspace((unsigned char)optarg[i]) || iscntrl((unsigned char)optarg[i])) { nut_diag_err("header key contains invalid characters."); return -1; }
                }
            ...
            default:  if (optind > 0 && optind <= argc) {
                          nut_diag_err("option '%s' unrecognized or invalid.", argv[optind - 1]);
                      } else {
                          nut_diag_err("option unrecognized or invalid.");
                      }
                      nut_diag_hint("run 'nut --help' for usage."); return NUT_ERR_ARG;
        }
    }
 
    if (args->tls12 && args->tls13) { nut_diag_err("conflicting TLS versions"); return NUT_ERR_ARG; }
    if (args->ping && args->dry_run) { nut_diag_err("--ping and --dry-run are mutually exclusive"); return NUT_ERR_ARG; }
    if (args->ping && args->download) { nut_diag_err("--ping and --download are mutually exclusive"); return NUT_ERR_ARG; }
    if (args->resolve && args->download) { nut_diag_err("--resolve and --download are mutually exclusive"); return NUT_ERR_ARG; }
    if (args->no_verify && args->cacert) { nut_diag_warn("--insecure and --cacert used together; --cacert takes precedence."); }
    if (args->resume && !args->download && !args->output) { nut_diag_err("--resume requires --download or -o"); return NUT_ERR_ARG; }
 
    int rem = argc - optind;
    if (rem <= 0) { nut_diag_err("no URL specified!"); return NUT_ERR_ARG; }
    if (rem > 1) { nut_diag_err("only a single URL target is supported. Multiple targets specified."); return NUT_ERR_ARG; }
 
    *url = nut_normalize_url(argv[optind]);
    nut_cli_infer_method(args);

    if (args->data && (nut_strcasecmp(args->method, "GET") == 0 || nut_strcasecmp(args->method, "HEAD") == 0)) {
        nut_diag_warn("sending body with %s is non-standard.", args->method);
    }
    if (nut_strcasecmp(args->method, "HEAD") == 0) args->include = true;
