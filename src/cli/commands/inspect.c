...
int nurl_cmd_inspect(const char *url, const CommonArgs *common) {
    ...
    if (!common->no_auth) {
        if ((common->bearer || common->token || common->user) && !nurl_utils_has_header(common->header, common->header_count, "Authorization")) {
            printf("> Authorization: [hidden]\n");
        }
    }

    if (common->json && !nurl_utils_has_header(common->header, common->header_count, "Content-Type")) printf("> Content-Type: application/json\n");

    size_t body_len = (common->data) ? (common->data_len > 0 ? common->data_len : strlen(common->data)) : 0;
    if (body_len > 0 && !nurl_utils_has_header(common->header, common->header_count, "Content-Length")) printf("> Content-Length: %zu\n", body_len);
