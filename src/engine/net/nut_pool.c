...
nut_err_t nut_pool_acquire(NutConnPool *pool, const char *host, int port, bool is_tls, const NutRequest *req, NutStream **stream) {
    if (!pool) return NUT_ERR_GENERIC;
    ...
    for (int i = 0; i < NUT_POOL_MAX; i++) {
        ...
        if (e->stream && !e->in_use && e->port == port && e->is_tls == is_tls && strcmp(e->host, host) == 0) {
            ...
            if (now - e->last_used > 60) {
                ...
                if (e->stream->tls) nut_tls_free(e->stream->tls);
                nut_net_close(e->stream->fd);
                nut_stream_free(e->stream);
                e->stream = NULL;
            } else if (!nut_net_is_alive(e->stream->fd)) {
                if (req->verbose && !req->silent) {
    ...
    int fd = nut_net_connect_proxy_ex(host, port, req->proxy, req->proxy_user, req->no_proxy, req->connect_to, req->connect_timeout_sec, &conn_err);
    if (fd < 0) {
        return conn_err;
    }
    if (req->read_timeout_sec > 0) {
        nut_net_set_timeout(fd, req->read_timeout_sec);
    }

    nut_tls_t *t = NULL;
    if (is_tls) {
        t = nut_tls_create(req->tls_verify, req->cacert, req->cert, req->key, req->tls_version == 12, req->tls_version == 13);
        if (!t) {
            nut_net_close(fd);
            return NUT_ERR_TLS;
        }

        if (nut_tls_handshake(t, fd, host) != 0) {
            nut_tls_free(t);
            nut_net_close(fd);
            return NUT_ERR_TLS_HANDSHAKE;
        }
    }

    NutStream *s = nut_stream_new(fd, t);
    if (!s) {
        if (t) nut_tls_free(t);
        nut_net_close(fd);
        return NUT_ERR_OOM;
...
void nut_pool_release(NutConnPool *pool, const char *host, int port, NutStream *stream) {
    if (!pool) return;
    (void)host;
    (void)port;
    for (int i = 0; i < NUT_POOL_MAX; i++) {
    ...
    if (stream->tls) nut_tls_free(stream->tls);
    nut_net_close(stream->fd);
