...
void nut_multipart_add_file(NutMultipart *m, const char *field_name, const char *filepath, const char *mime_type) {
...
void nut_multipart_into_request(NutMultipart *m, NutRequest *req) {
    ...
    for (size_t i = 0; i < m->count; i++) {
        ...
        req->body_parts[req->body_parts_count].type = NUT_BODY_PART_MEM;
        ...
        if (p->is_file) {
            ...
            req->body_parts[req->body_parts_count].type = NUT_BODY_PART_FILE;
    ...
    nut_headermap_set(req->headers, "Content-Type", nut_multipart_content_type(m));
    return;

oom_cleanup:
    /* Free any already-allocated parts and signal failure */
    for (size_t j = 0; j < req->body_parts_count; j++) {
        if (req->body_parts[j].type == NUT_BODY_PART_MEM) {
            free((void *)req->body_parts[j].data);
        } else if (req->body_parts[j].type == NUT_BODY_PART_FILE) {
