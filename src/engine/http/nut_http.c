...
nut_err_t nut_http_request(
    ...
    if (p->body_parts && p->body_parts_count > 0) {
        ...
        for (size_t i = 0; i < p->body_parts_count; i++) {
            if (p->body_parts[i].type == NUT_BODY_PART_MEM) {
                total_body_len += p->body_parts[i].len;
            } else if (p->body_parts[i].type == NUT_BODY_PART_FILE) {
                struct stat st;
                if (nut_stat(p->body_parts[i].filepath, &st) == 0) {
    ...
    NutBuf req_buf;
    nut_buf_init(&req_buf);

    bool has_user_agent = false;
    bool has_connection = false;
    if (p->extra_headers) {
        const char *eh = p->extra_headers;
        while ((eh = nut_strcasestr(eh, "User-Agent")) != NULL) {
        ...
        eh = p->extra_headers;
        while ((eh = nut_strcasestr(eh, "Connection")) != NULL) {
            if (eh == p->extra_headers || eh[-1] == '\n' || eh[-1] == '\r') {
                const char *colon = strchr(eh, ':');
                if (colon) {
                    has_connection = true;
    ...
    if (!nut_buf_printf(&req_buf, "%s %s HTTP/%s\r\nHost: %s\r\n", p->method, p->path, p->http10 ? "1.0" : "1.1", p->hostname)) {
        nut_buf_free(&req_buf);
        return NUT_ERR_OOM;
    }

    if (!has_user_agent) {
        if (!nut_buf_printf(&req_buf, "User-Agent: nurl/" NUT_VERSION "\r\n")) {
            nut_buf_free(&req_buf);
            return NUT_ERR_OOM;
        }
    }
    if (!has_connection) {
        if (!nut_buf_printf(&req_buf, "Connection: %s\r\n", p->http10 ? "close" : "keep-alive")) {
            nut_buf_free(&req_buf);
            return NUT_ERR_OOM;
        }
    }
    ...
    if (total_body_len > 0) {
        if (!nut_buf_printf(&req_buf, "Content-Length: %zu\r\n", total_body_len)) {
            nut_buf_free(&req_buf);
            return NUT_ERR_OOM;
        }
    }
    ...
    if (p->extra_headers) {
        if (!nut_buf_append(&req_buf, p->extra_headers, strlen(p->extra_headers))) {
            nut_buf_free(&req_buf);
            return NUT_ERR_OOM;
        }
    }
    ...
    if (!nut_buf_append(&req_buf, "\r\n", 2)) {
        nut_buf_free(&req_buf);
        return NUT_ERR_OOM;
    }

    // Send HTTP Headers
    int w_headers = nut_stream_write(stream, req_buf.data, req_buf.len);
    if (w_headers <= 0) {
        nut_err_t err = (w_headers < 0) ? (nut_err_t)(-w_headers) : NUT_ERR_NETWORK;
        nut_buf_free(&req_buf);
        return err;
    }
    nut_buf_free(&req_buf);
    ...
    if (p->body_parts && p->body_parts_count > 0) {
        for (size_t i = 0; i < p->body_parts_count; i++) {
            ...
            if (part->type == NUT_BODY_PART_MEM) {
                if (part->data && part->len > 0) {
                    int wn = nut_stream_write(stream, part->data, part->len);
                    if (wn <= 0) {
                        return (wn < 0) ? (nut_err_t)(-wn) : NUT_ERR_NETWORK;
                    }
                }
            } else if (part->type == NUT_BODY_PART_FILE) {
                if (part->filepath) {
                    FILE *bf = fopen(part->filepath, "rb");
                    if (!bf) {
                        nut_diag_err("could not open '%s' for reading: %s", part->filepath, strerror(errno));
                        return NUT_ERR_IO;
                    }
                    char send_buf[65536];
                    size_t r;
                    while ((r = fread(send_buf, 1, sizeof(send_buf), bf)) > 0) {
                        int wn = nut_stream_write(stream, send_buf, r);
                        if (wn <= 0) {
                            nut_err_t err = (wn < 0) ? (nut_err_t)(-wn) : NUT_ERR_NETWORK;
                            fclose(bf);
                            return err;
                        }
                    }
        ...
        int wn = nut_stream_write(stream, p->body, p->body_len);
        if (wn <= 0) {
            return (wn < 0) ? (nut_err_t)(-wn) : NUT_ERR_NETWORK;
        }
    }
    ...
    nut_http_response_t *res = calloc(1, sizeof(nut_http_response_t));
    if (!res) return NUT_ERR_OOM;

    char line_buf[8192];
    int line_len;

    // Read Status Line
    line_len = nut_stream_read_line(stream, line_buf, sizeof(line_buf));
    if (line_len <= 0) {
        nut_err_t err = (line_len < 0) ? (nut_err_t)(-line_len) : NUT_ERR_NETWORK;
    ...
    while (1) {
        ...
        if (colon) {
            *colon = '\0';
            ...
            if (nut_strcasecmp(key, "Transfer-Encoding") == 0 && nut_strcasecmp(val, "chunked") == 0) {
                is_chunked = true;
            } else if (nut_strcasecmp(key, "Content-Length") == 0) {
                content_len = strtoul(val, NULL, 10);
            } else if (nut_strcasecmp(key, "Content-Range") == 0) {
    ...
    if (nut_strcasecmp(p->method, "HEAD") == 0) {
    ...
    if (is_chunked) {
        ...
        while (1) {
            ...
            int size_line_len = nut_stream_read_line(stream, chunk_size_buf, sizeof(chunk_size_buf));
            if (size_line_len <= 0) {
                if (size_line_len < 0) {
                    if (body_buf) free(body_buf);
                    nut_http_response_free(res);
                    return (nut_err_t)(-size_line_len);
                }
                break; // Premature EOF
            }
            ...
            if (chunk_size == 0) {
                ...
                char dummy[2];
                int dr = nut_stream_read_exact(stream, dummy, 2);
                if (dr <= 0) {
                    if (body_buf) free(body_buf);
                    nut_http_response_free(res);
                    return (dr < 0) ? (nut_err_t)(-dr) : NUT_ERR_NETWORK;
                }
                break; // Finished
            }
            ...
            while (read_chunk < chunk_size) {
                ...
                int n = nut_stream_read(stream, chunk_buf, to_read);
                if (n <= 0) {
                    nut_err_t err = (n < 0) ? (nut_err_t)(-n) : NUT_ERR_NETWORK;
                    if (body_buf) free(body_buf);
                    nut_http_response_free(res);
                    return err;
                }
        ...
        while (body_len < content_len) {
            ...
            if (n <= 0) {
                if (body_buf) free(body_buf);
                nut_http_response_free(res);
                return (n < 0) ? (nut_err_t)(-n) : NUT_ERR_NETWORK;
            }
        ...
        char chunk_buf[4096];
        int n;
        while ((n = nut_stream_read(stream, chunk_buf, sizeof(chunk_buf))) != 0) {
            if (n < 0) {
                if (body_buf) free(body_buf);
                nut_http_response_free(res);
                return (nut_err_t)(-n);
