...
nut_err_t nut_headermap_set(NutHeaderMap *m, const char *key, const char *value) {
    if (!m || !key || !value) return NUT_ERR_GENERIC;

    char *canon_key = strdup(key);
    if (!canon_key) return NUT_ERR_OOM;
    canonicalize_header_key(canon_key);

    for (size_t i = 0; i < m->count; i++) {
        if (nut_strcasecmp(m->keys[i], canon_key) == 0) {
            free(canon_key);
            char *new_val = strdup(value);
            if (!new_val) return NUT_ERR_OOM;
            free(m->values[i]);
            m->values[i] = new_val;
            return NUT_OK;
        }
...
bool nut_headermap_has(const NutHeaderMap *m, const char *key) {
    ...
    for (size_t i = 0; i < m->count; i++) {
        if (nut_strcasecmp(m->keys[i], key) == 0) {
            return true;
        }
    }
    return false;
...
char *nut_headermap_serialize(const NutHeaderMap *m) {
    ...
    nut_buf_init(&b);
    for (size_t i = 0; i < m->count; i++) {
        nut_buf_printf(&b, "%s: %s\r\n", m->keys[i], m->values[i]);
    }
    return nut_buf_take(&b);
}
...
nut_err_t nut_headermap_add_raw(NutHeaderMap *m, const char *line) {
    if (!m || !line) return NUT_ERR_GENERIC;
    const char *colon = strchr(line, ':');
    if (!colon) return NUT_ERR_GENERIC;

    size_t key_len = colon - line;
    char *key = malloc(key_len + 1);
    if (!key) return NUT_ERR_OOM;
    ...
    nut_err_t err = nut_headermap_append(m, key, value);
    free(key);
    free(value);
    return err;
}
