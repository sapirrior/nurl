...
int nut_utils_parse_url(const char *url, char **scheme, char **host, int *port, char **path) {
    if (!url || strlen(url) == 0) {
        return -1;
    }
...
const char *nut_utils_redact_header(const char *key, const char *value) {
    if (!key) return value;
    if (nut_strcasecmp(key, "authorization") == 0 || nut_strcasecmp(key, "cookie") == 0) {
        return "[hidden]";
    }
    return value;
}

char *nut_utils_trim(char *str) {
    if (!str) return NULL;
...
char *nut_utils_base64_encode(const unsigned char *src, size_t len) {
...
double nut_utils_get_time_sec(void) {
#ifdef _WIN32
    FILETIME ft;
...
char *nut_utils_read_stdin(size_t *out_len) {
