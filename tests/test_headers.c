...
void test_header_serialization(void) {
    NutHeaderMap *m = nut_headermap_new();
    ASSERT_NOT_NULL(m);
    
    nut_headermap_set(m, "Content-Type", "application/json");
    nut_headermap_set(m, "accept", "text/html"); // case-insensitive handling
    
    char *serialized = nut_headermap_serialize(m);
